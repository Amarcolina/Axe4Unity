...
namespace Axe4Unity {
  ...
  public class AxeSpeedRunFixer : MonoBehaviour {

    public AxeRunner RunnerOld;
    public AxeRunner RunnerNew;
    ...
    private List<(int x, int y)> _playerPosList = new();
    private List<(int x, int y)> _playerPosListReference = new();
    ...
    private void Start() {
      ...
      _playerPosListReference = null;
      _playerVelListReference = null;
      GetCost(RunnerOld, _currRun);
      _playerPosListReference = _playerPosList.ToList();
      _playerVelListReference = _playerVelList.ToList();

      CurrCost = GetCost(RunnerNew, _currRun);
      StartCost = CurrCost;
      BestCost = CurrCost;
    }
    ...
    private void Update() {
      ...
      while (true) {
        ...
        int neighborCost = GetCost(RunnerNew, _neighborRun);
    ...
    public int GetCost(AxeRunner runner, List<int> run) {
      _playerPosList.Clear();
      ...
      int playerYAddr = runner.Machine.AddressOfName("Y");

      int playerVelXAddr = runner.Machine.AddressOfName("A");
      int playerVelYAddr = runner.Machine.AddressOfName("B");
      ...
      foreach (var key in run) {
        ...
        int playerY = runner.Machine.State.Read_U16(playerYAddr);
        int playerA = runner.Machine.State.Read_U16(playerVelXAddr);
        int playerB = runner.Machine.State.Read_U16(playerVelYAddr);

        if (_playerPosList != null) {
          _playerPosList.Add((playerX, playerY));
          _playerVelList.Add((playerA, playerB));
        }
      ...
      if (_playerPosListReference == null) {
        throw new System.Exception();
      }

      int cost = _playerPosList.Count;
      for (int i = 0; i < _playerPosListReference.Count; i++) {
        (int x, int y) posNew;
        (int x, int y) velNew;
        if (i < _playerPosList.Count) {
          posNew = _playerPosList[i];
          velNew = _playerVelList[i];
        } else {
          posNew = (-4000, -4000);
          velNew = (-4000, -4000);
        }

        var posOld = _playerPosListReference[i];
        var velOld = _playerVelListReference[i];

        int dPos = 0;
        dPos += Mathf.Abs(posOld.x - posNew.x);
        dPos += Mathf.Abs(posOld.y - posNew.y);
